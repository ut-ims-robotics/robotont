...
void format_input(std_msgs::String msg){
	...
	//conversion factor
	//float const cf = 2*M_PI/encoderEdgesPerMotorRevolution;
	...
	ROS_INFO_STREAM("matrix :" << odomMatrixInv);

	vx = odomMatrixInv(0,0)*phi0 + odomMatrixInv(0,1)*phi1 + odomMatrixInv(0,2)*phi2;
	vy = odomMatrixInv(1,0)*phi0 + odomMatrixInv(1,1)*phi1 + odomMatrixInv(1,2)*phi2;
	vth = odomMatrixInv(2,0)*phi0 + odomMatrixInv(2,1)*phi1 + odomMatrixInv(2,2)*phi2;
	
	/*
	vx = wheelRadius * ((2*phi0/3) - (phi1/3) - (phi2/3));
	vy = wheelRadius * ((phi1/sqrt(3)) - (phi2/sqrt(3)));
	vth = (wheelRadius/(3*wheelDistanceFromCenter)) * (phi0 + phi1 + phi2);
	*/
...
int main(int argc, char** argv) {
	...
	ros::Subscriber write_sub = nh.subscribe("serial_write", 50, write_callback);
